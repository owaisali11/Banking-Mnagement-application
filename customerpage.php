...
<head>
    ...
    <link rel="stylesheet" href="./Styles/customerpagee.css" />
    <script defer src="customerpag.js"></script>
...
if(isset($_POST['submit_amount'])){
    ...
    if($transferAcc != "" && $amount < $checkBalance[0] && $amount>0 ){
    ...
    $_SESSION['msg1'] = "Money Successfully Transfered";
        ...
        $_SESSION['msg1']="cannot transfer money";
    }
...
// transaction history table
  ...
  if(isset($_POST['submit_loan'])){
   ...
   if($checkBalance[0]  >= $loan_amount && $loan_amount>0 ){
    ...
    $updateAccount = mysqli_query($conn,"UPDATE accounts set balance = balance + '$loan_amount' ");
        ...
        $_SESSION['msg1']="cannot give loan";
    }
...
<button class="btn--close-modal-closeMsg">&times;</button>
    ...
    <main class="app">
      ...
      <h2 margin="0px auto" color="Black" >Transaction History</h2>
        ...
        <tr>
        <?php
        $sql= "select acc_no,transfer_to,amount,date_time from transaction where acc_no = (SELECT acc_no from accounts where acc_no=(select acc_No from customers where username='$loggedUsername'));";
        $result= mysqli_query($conn, $sql);
